    ...
    <div className="card space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-2xl font-bold font-display ${
          pct >= 100 ? "text-emerald-400" : pct >= 75 ? "text-amber-400" : "text-brand-400"
        }`}>
          %{successRate}
        </span>
      </div>
      ...
      <div className="flex items-end justify-between text-sm">
        <div>
          ...
          <p className="text-stone-100 font-semibold font-display">{formatCurrency(currentSales)}</p>
        </div>
        <div className="text-right">
          <p className="text-stone-500 text-xs mb-0.5">Hedef</p>
          <p className="text-stone-400 font-semibold font-display">{formatCurrency(monthlyTarget)}</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        {pct >= 100 && (
          <div className="absolute -top-1 right-0 text-emerald-400 text-xs font-bold">
            ✓ Hedef aşıldı!
          </div>
        )}
      </div>
              ...
              .map(([type, count]) => (
                <div key={type} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 bg-surface-3 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${ACTIVITY_COLORS[type] ?? "bg-stone-500"} transition-all duration-700`}
                      style={{ width: `${(count / totalActivity) * 100}%` }}
                    />
                  </div>
                  <span className="text-stone-400 text-xs w-4 text-right">{count}</span>
                </div>
              ))}
          </div>
        </div>
