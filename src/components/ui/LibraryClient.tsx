...
interface Article {
  ...
  author?: string; published_year?: number; cover_image_url?: string;
  read_time_minutes?: number; featured_quote?: string;
}
...
export default function LibraryClient({
  ...
  const dictCats = useMemo(() =>
    ["Tümü", ...Array.from(new Set(dictionary.map(d => d.category))).sort()], [dictionary]);
  ...
  const articleCats = useMemo(() =>
    ["Tümü", ...Array.from(new Set(articles.map(a => a.category))).sort()], [articles]);
  ...
  // Kart tıklama — mobilde expand, masaüstünde hover
  function handleCardClick(id: string) {
    if (!isMobile) return;
    setExpandedCard(prev => prev === id ? null : id);
  }

  // Liste overlay — sadece masaüstünde
  ...
  function handleCardMouseEnter(e: React.MouseEvent<HTMLDivElement>) {
    if (isMobile) return;
    ...
    if (view === "card" && !isMobile) {
      setTimeout(() => {
        ...
        grid.querySelectorAll<HTMLDivElement>(".dict-card").forEach(card => {
          ...
          card.style.transformOrigin = r.right + 80 > gRight ? "right top" : "left top";
        });
      }, 60);
    }
  }, [view, filteredDict, isMobile]);
    ...
    <div className="space-y-5 animate-fade-up" style={{ animationDelay: "0.1s" }}>
      ...
      <div style={{ display: "flex", border: "0.5px solid #e2e8f0", borderRadius: 12, overflow: "hidden", backgroundColor: "#f8fafc" }}>
        {([["dictionary", "📖 Satış Sözlüğü"], ["articles", "📚 Satış Kütüphanesi"]] as [Tab, string][]).map(([t, l]) => (
          <button key={t} onClick={() => { setTab(t); setSearch(""); setSelectedArticle(null); setActiveCat("Tümü"); setActiveLetter(""); setShowFavsOnly(false); setExpandedCard(null); }}
            style={{ flex: 1, padding: "12px 8px", border: "none", borderRight: t === "dictionary" ? "0.5px solid #e2e8f0" : "none", backgroundColor: tab === t ? "#fff" : "transparent", color: tab === t ? "#00abaa" : "#64748b", fontSize: "clamp(12px, 3vw, 14px)", fontWeight: 500, cursor: "pointer", transition: "all .15s", boxShadow: tab === t ? "inset 0 -2px 0 #00abaa" : "none", whiteSpace: "nowrap" }}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "stretch", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, backgroundColor: "#fff", border: "0.5px solid #e2e8f0", borderRadius: 12, padding: "10px 14px", flex: 1, minWidth: 0 }}>
          <span style={{ color: "#94a3b8", fontSize: 16, flexShrink: 0 }}>🔍</span>
          <input type="text"
            style={{ border: "none", outline: "none", flex: 1, fontSize: 14, backgroundColor: "transparent", color: "#1e293b", minWidth: 0 }}
            placeholder={tab === "dictionary" ? "Terim ara…" : "Kitap ara…"}
            value={search} onChange={e => setSearch(e.target.value)} />
          {search && <button onClick={() => setSearch("")} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 13, flexShrink: 0 }}>✕</button>}
        </div>
        <div style={{ display: "flex", flexDirection: "column", border: "0.5px solid #e2e8f0", borderRadius: 12, overflow: "hidden", flexShrink: 0 }}>
          {(["card", "list"] as View[]).map((v, i) => (
            <button key={v} onClick={() => { setView(v); setExpandedCard(null); }}
      ...
      {tab === "dictionary" && (
        <div className="space-y-4">
          ...
          {/* A-Z — yatay scroll ile taşmayı önle */}
          <div style={{ display: "flex", gap: 4, overflowX: "auto", paddingBottom: 4, WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
            {letters.map(l => (
              <button key={l} onClick={() => toggleLetter(l)}
                style={{ width: 28, height: 28, borderRadius: 6, border: "0.5px solid #e2e8f0", backgroundColor: activeLetter === l ? "#00abaa" : "transparent", color: activeLetter === l ? "#fff" : "#64748b", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 500, transition: "all .15s", flexShrink: 0 }}>
                {l}
              </button>
            ))}
          </div>
          ...
          {filteredDict.length === 0 ? (
            ...
            // KART GÖRÜNÜMÜ
            <div id="dictGrid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
              {filteredDict.map(term => {
                const { name, full } = parseTerm(term.term_name);
                const isExpanded = expandedCard === term.id;
                return (
                  <div key={term.id} style={{ position: "relative", height: isMobile ? "auto" : 140 }}>
                    <div className="dict-card"
                      ...
                      style={{
                        ...
                        background: isExpanded ? "#f0fffe" : "#fff",
                        border: isExpanded ? "0.5px solid #00abaa" : "0.5px solid #e2e8f0",
                        borderRadius: 14, padding: 16, cursor: isMobile ? "pointer" : "default",
                        zIndex: 1, display: "flex", flexDirection: "column", gap: 6,
                        overflow: "hidden", width: "100%",
                        boxShadow: isExpanded ? "0 4px 16px rgba(0,171,170,0.15)" : "none",
                        transition: "border-color .2s, box-shadow .25s, width .28s cubic-bezier(0.22,1,0.36,1)",
                      }}>
                      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                        <span style={{ fontSize: 14, fontWeight: 600, color: "#1e293b", lineHeight: 1.3, flex: 1 }}>{name}</span>
                      ...
                      {full && <div style={{ fontSize: 11, color: "#94a3b8", fontStyle: "italic", lineHeight: 1.4 }}>{full}</div>}
                      {/* Açıklama: mobilde expanded, masaüstünde hover */}
                      <div className="dict-desc" style={{
                        fontSize: 12, color: "#475569", lineHeight: 1.6,
                        maxHeight: isExpanded ? "200px" : 0,
                        overflow: "hidden", opacity: isExpanded ? 1 : 0,
                      ...
                      <div style={{ marginTop: "auto", paddingTop: 6 }}>
                        <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 99, fontSize: 11, fontWeight: 500, backgroundColor: "#db0962", color: "#fff" }}>{term.category}</span>
                      </div>
                    </div>
                  </div>
                );
            ...
            // LİSTE GÖRÜNÜMÜ — mobilde yatay scroll
            <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
              <div ref={listWrapRef} style={{ border: "0.5px solid #e2e8f0", borderRadius: 12, backgroundColor: "#fff", position: "relative", overflow: "visible", minWidth: 480 }}>
                <div ref={overlayRef}
                  onMouseEnter={() => clearTimeout(hideTimer.current ?? undefined)}
                  onMouseLeave={() => { hideTimer.current = setTimeout(() => { if (overlayRef.current) overlayRef.current.style.display = "none"; }, 100); }}
      ...
      {tab === "articles" && !selectedArticle && (
        <div className="space-y-5">
          ...
          {filteredArticles.length === 0 ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 14 }}>
              {filteredArticles.map(article => (
                <button key={article.id} onClick={() => setSelectedArticle(article)}
                  style={{ textAlign: "left", cursor: "pointer", border: "none", backgroundColor: "transparent", padding: 0, width: "100%" }}>
                  <div style={{ backgroundColor: "#fff", border: "0.5px solid #e2e8f0", borderRadius: 12, overflow: "hidden", height: "100%" }}
                    onMouseEnter={e => { if (!isMobile) { const el = e.currentTarget as HTMLDivElement; el.style.boxShadow = "0 8px 24px rgba(0,171,170,0.15)"; el.style.transform = "translateY(-2px)"; }}}
                    onMouseLeave={e => { if (!isMobile) { const el = e.currentTarget as HTMLDivElement; el.style.boxShadow = ""; el.style.transform = ""; }}}>
                    <div style={{ height: 140, backgroundColor: "#e0f7f7", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
                      {article.cover_image_url ? <img src={article.cover_image_url} alt={article.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 36 }}>📚</span>}
                      <span style={{ position: "absolute", top: 8, right: 8, fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 99, backgroundColor: "rgba(255,255,255,0.92)", color: "#00abaa", border: "1px solid #b2eded" }}>{article.category}</span>
                    </div>
                    <div style={{ padding: 12 }}>
                      <h3 style={{ fontFamily: "'Chalet', sans-serif", fontWeight: 700, color: "#1e293b", fontSize: 13, marginBottom: 3, lineHeight: 1.3 }}>{article.title}</h3>
                      {article.author && <p style={{ color: "#64748b", fontSize: 12 }}>{article.author}</p>}
                      {article.read_time_minutes && <p style={{ color: "#94a3b8", fontSize: 11, marginTop: 3 }}>⏱ {article.read_time_minutes} dk</p>}
                    </div>
                  </div>
                </button>
              ))}
            ...
            // Kitap listesi — mobilde scroll
            <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" } as React.CSSProperties}>
              <div style={{ border: "0.5px solid #e2e8f0", borderRadius: 12, overflow: "hidden", backgroundColor: "#fff", minWidth: 480 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                  <thead>
                    <tr style={{ backgroundColor: "#00abaa" }}>
      ...
      {tab === "articles" && selectedArticle && (
        <div className="space-y-6 animate-fade-up">
          ...
          <div className="card" style={{ padding: 0, overflow: "hidden" }}>
            {/* Kapak + meta — mobilde alt alta */}
            <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", flexWrap: "wrap" }}>
              <div style={{ width: isMobile ? "100%" : 200, height: isMobile ? 220 : undefined, minHeight: isMobile ? undefined : 260, backgroundColor: "#e0f7f7", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {selectedArticle.cover_image_url
                  ? <img src={selectedArticle.cover_image_url} alt={selectedArticle.title} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  : <span style={{ fontSize: 64 }}>📚</span>}
              </div>
              <div style={{ flex: 1, padding: isMobile ? "20px 20px 16px" : "28px 32px", minWidth: 0 }}>
                ...
                <h1 style={{ fontFamily: "'Chalet', sans-serif", fontSize: "clamp(18px, 5vw, 24px)", fontWeight: 700, color: "#1e293b", marginBottom: 8, lineHeight: 1.3 }}>{selectedArticle.title}</h1>
                {selectedArticle.author && <p style={{ color: "#475569", fontSize: 14, marginBottom: 4 }}>✍️ {selectedArticle.author}</p>}
                {selectedArticle.published_year && <p style={{ color: "#94a3b8", fontSize: 13, marginBottom: 4 }}>📅 {selectedArticle.published_year}</p>}
                {selectedArticle.read_time_minutes && <p style={{ color: "#94a3b8", fontSize: 13 }}>⏱ {selectedArticle.read_time_minutes} dakika okuma</p>}
              </div>
            </div>
            {selectedArticle.featured_quote && (
              <div style={{ margin: isMobile ? "0 16px 20px" : "0 32px 24px", padding: "16px 20px", backgroundColor: "#f0fffe", border: "1px solid #b2eded", borderRadius: 12, borderLeft: "4px solid #00abaa" }}>
                <p style={{ fontFamily: "'Chalet', sans-serif", color: "#1e293b", fontSize: 15, lineHeight: 1.7, fontStyle: "italic" }}>"{selectedArticle.featured_quote}"</p>
              </div>
            )}
            <div style={{ padding: isMobile ? "0 16px 24px" : "0 32px 32px" }}>
              <h2 style={{ fontFamily: "'Chalet', sans-serif", fontSize: 16, fontWeight: 700, color: "#1e293b", marginBottom: 14 }}>Kitap Özeti</h2>
              <div style={{ color: "#475569", fontSize: 14, lineHeight: 1.9, whiteSpace: "pre-wrap" }}>{selectedArticle.content}</div>
            </div>
          </div>
        </div>
      )}
