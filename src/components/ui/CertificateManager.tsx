    ...
    <div className="space-y-6">
      {msg && (
        <div style={{ display: "flex", gap: 8, borderRadius: 12, padding: "12px 16px", backgroundColor: msg.type === "error" ? "#fef2f2" : "#f0fdf4", border: `1px solid ${msg.type === "error" ? "#fecaca" : "#bbf7d0"}` }}>
          <span style={{ color: msg.type === "error" ? "#ef4444" : "#10b981" }}>{msg.type === "error" ? "⚠" : "✓"}</span>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        <select style={{ ...inp, width: 200 }} value={filterUser} onChange={e => setFilterUser(e.target.value)}>
          <option value="">Tüm Personel</option>
          {profiles.map(p => <option key={p.id} value={p.id}>{p.full_name}</option>)}
        </select>
        <span style={{ color: "#94a3b8", fontSize: 13 }}>{filtered.length} sertifika</span>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          <button onClick={exportExcel} style={{ padding: "8px 16px", borderRadius: 10, border: "1px solid #e2e8f0", backgroundColor: "#fff", color: "#475569", fontSize: 13, cursor: "pointer" }}>
            ⬇ Excel
          </button>
          <button onClick={() => setShowAdd(!showAdd)}
            style={{ padding: "8px 16px", borderRadius: 10, border: "none", backgroundColor: "#00abaa", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ backgroundColor: "#00abaa" }}>
                  {["Personel", "Sertifika", "Sınav", "Veriliş Tarihi", "Dosya", "İşlem"].map(h => (
              ...
              <tbody>
                {filtered.map(cert => (
                  <tr key={cert.id}
                    ...
                    <td style={{ padding: "12px 14px", fontWeight: 500, color: "#1e293b" }}>{cert.title}</td>
                    <td style={{ padding: "12px 14px", color: "#64748b", fontSize: 13 }}>{cert.exams?.title ?? "—"}</td>
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => window.open(cert.file_url, "_blank")}
                          style={{ fontSize: 12, padding: "4px 10px", borderRadius: 8, border: "1px solid #e2e8f0", backgroundColor: "#fff", color: "#475569", cursor: "pointer" }}>
                          Görüntüle
                        </button>
