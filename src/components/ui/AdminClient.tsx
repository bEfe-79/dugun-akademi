...
export default function AdminClient({
  ...
  function exportCSV() {
    ...
    a.href = url;
    a.download = `gunlukler-${filterDate || "tum"}.csv`;
    a.click();
  ...
  async function exportExcel() {
    ...
    ws["!cols"] = [
      { wch: 20 }, { wch: 12 }, { wch: 8 },
      { wch: 12 }, { wch: 50 }, { wch: 20 },
    ];
    const wb = utils.book_new();
    utils.book_append_sheet(wb, ws, "Günlükler");
    writeFile(wb, `gunlukler-${filterDate || "tum"}.xlsx`);
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        {[
          ...
          { label: "Bugün Log", value: String(logs.filter(l => l.log_date === todayStr).length), icon: "📋" },
          { label: "Görüntülenen", value: String(filteredLogs.length), icon: "🔍" },
        ].map((c) => (
          <div key={c.label} className={`card ${c.accent ? "border-emerald-500/20 bg-emerald-900/10" : ""}`}>
            ...
            <p className={`text-2xl font-bold font-display ${c.accent ? "text-emerald-400" : "text-stone-100"}`}>
              {c.value}
            </p>
          </div>
        ))}
          ...
          <div className="card flex flex-wrap gap-3 items-end">
            ...
            <div>
              ...
              >
                <option value="">Tümü</option>
                {(
                  Object.entries(ACTIVITY_META) as [
                    ActivityType,
            ...
            <div className="flex gap-2 ml-auto">
              <button onClick={exportCSV} className="btn-ghost text-xs px-4 py-2">
                ⬇ CSV
              </button>
              <button onClick={exportExcel} className="btn-primary text-xs px-4 py-2">
                ⬇ Excel
              </button>
            </div>
          </div>
                ...
                <thead>
                  <tr className="border-b border-surface-3 bg-surface-2">
                    {["Personel", "Tarih", "Saat", "Aktivite", "Not"].map((h) => (
                      <th
                        key={h}
                        className="text-left px-4 py-3 text-stone-500 font-semibold text-xs uppercase tracking-wider"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-surface-3">
                  {filteredLogs.length === 0 ? (
                    ...
                    filteredLogs.map((log) => {
                      const meta =
                        ACTIVITY_META[log.activity_type as ActivityType] ??
                        ACTIVITY_META.other;
                      return (
                        <tr
                          key={log.id}
                        ...
                        >
                          ...
                          <td
                            className="px-4 py-3 text-stone-400 max-w-xs truncate"
        ...
        <div className="card p-0 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-3 bg-surface-2">
                  {["Personel", "Rol", "Aylık Hedef", "Gerçekleşen", "Başarı", "Son Giriş"].map(
                    (h) => (
                      <th
                        key={h}
                        className="text-left px-4 py-3 text-stone-500 font-semibold text-xs uppercase tracking-wider"
              ...
              <tbody className="divide-y divide-surface-3">
                {profiles.map((p) => {
                  const rate =
                    p.monthly_target > 0
                      ? Math.round(
                          (p.current_sales / p.monthly_target) * 100
                        )
                      : 0;
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-surface-2 transition-colors">
                      ...
                      <td className="px-4 py-3 text-stone-400">
                        {formatCurrency(p.monthly_target)}
                      </td>
                      <td className="px-4 py-3 text-stone-200 font-medium">
                        {formatCurrency(p.current_sales)}
                      </td>
                      <td className="px-4 py-3">
