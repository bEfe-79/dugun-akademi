...
export default function LogForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!logContent.trim() || !logTime) {
      setError("Saat ve not alanları zorunludur.");
      return;
    }
    ...
    if (dbErr) {
      setError("Kayıt sırasında hata oluştu: " + dbErr.message);
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {success && (
          <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3">
            ...
            <p className="text-emerald-300 text-sm">Kayıt başarıyla eklendi!</p>
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-stone-600 text-xs">
            Kaydedildikten sonra düzenlenemez veya silinemez.
          </p>
        </div>
      </form>
    </div>
