...
export default function LogForm({ today }: { today: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!userId) { setError("Oturum bulunamadı."); return; }
    ...
    if (dbErr) { setError("Kayıt sırasında hata: " + dbErr.message); setSaving(false); return; }

    setLogContent("");
    setSuccess(true);
    setSaving(false);
    // Sayfayı yenile — kayıt listesi güncellensin
    router.refresh();
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {success && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, backgroundColor: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: "12px 16px" }}>
            ...
            <p style={{ color: "#059669", fontSize: 14 }}>Kayıt başarıyla eklendi!</p>
          </div>
        )}

        <div className="flex items-center gap-3">
          <button type="submit" disabled={saving || !userId} className="btn-primary">
            {saving ? (
