  ...
  return (
    <aside className="hidden md:flex w-60 shrink-0 flex-col bg-surface-1 border-r border-surface-3">
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-surface-3">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-lg shrink-0">
          💍
        </div>
        <div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        <p className="px-3 mb-2 text-[10px] font-bold text-stone-600 uppercase tracking-widest">
          Menü
        </p>
        {NAV.map((item) => (
          <NavItem
        ...
        {profile?.role === "admin" && (
          <>
            <p className="px-3 mt-5 mb-2 text-[10px] font-bold text-stone-600 uppercase tracking-widest">
              Yönetim
            </p>
            {ADMIN_NAV.map((item) => (
              <NavItem
      ...
      <div className="px-4 py-4 border-t border-surface-3">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-brand-600/30 border border-brand-500/30 flex items-center justify-center text-brand-400 font-bold text-sm shrink-0">
            {profile?.full_name?.[0] ?? "?"}
          </div>
...
function NavItem({
  ...
  return (
    ...
    >
      <span className={`text-base ${active ? "text-brand-400" : "text-stone-600"}`}>
        {icon}
      </span>
      {label}
    </Link>
