...
const TYPE_META: Record<string, { icon: string; color: string; bg: string; label: string }> = {
  ...
  admin_message: { icon: "✉️", color: "#db0962", bg: "#fce7f0", label: "Mesaj" },
};
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const [profileRes, annoRes, logsRes, todayRes, trainTotalRes, compRes, subRes, assignRes, allSubsRes] = await Promise.all([
        ...
        // Duyurular: is_active=true VE (target_user_id NULL VEYA target_user_id = kendi id'si)
      ...
      const sorted = Object.entries(scoreMap)
        .map(([uid, scores]) => ({ uid, avg: Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) }))
        ...
        <div style={{ display: "contents" }} className="bento-grid">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <Link href="/logs" style={{ textDecoration: "none" }}>
              <div className="animate-fade-up" style={{ backgroundImage: "linear-gradient(150deg,#f0fffe,#e6f9f8)", borderRadius: 20, boxShadow: "0 8px 30px rgba(0,0,0,.04),0 2px 8px rgba(0,0,0,.03)", padding: 18, cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                  <div style={{ width: 42, height: 42, borderRadius: 13, backgroundColor: "#00abaa", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, boxShadow: "0 4px 12px rgba(0,171,170,.3)", flexShrink: 0 }}>📝</div>
                  <div>
            ...
            <div className="animate-fade-up" style={{ backgroundColor: "#fff", borderRadius: 20, boxShadow: "0 8px 30px rgba(0,0,0,.04),0 2px 8px rgba(0,0,0,.03)", padding: 18, flex: 1 }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".09em", marginBottom: 12 }}>Duyurular & Mesajlar</p>
